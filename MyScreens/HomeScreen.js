...
import {View, Text, FlatList, TouchableOpacity, Button, Image, StyleSheet, ScrollView } from 'react-native';
...
function Home(props) {
    ...
    const api = async()=>{
        const getApi = await axios.get('https://www.themealdb.com/api/json/v1/1/filter.php?a=Canadian')
         const getApiCoctail = await axios.get('https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=Ordinary_Drink')
...
const coctailProduct =(id)=>{
  ...
  return (
    <View style={{flex: 1, alignItems: 'center', justifyContent: 'center'}}>
      <Text style={{fontSize: 20, marginTop:20, color:'black', fontStyle: 'italic'}}>Meals of the day</Text>
      {/*<TouchableOpacity
        style={{
          marginTop: 20,
          width: 200,
      ...
      <Text style={{fontSize: 20, marginTop:20, color:'black', fontStyle: 'italic'}}>Cocktails</Text>
      <ScrollView style={{flex:1, marginTop:15}} horizontal={true} >
      {
          coctail.map(meal=> (
